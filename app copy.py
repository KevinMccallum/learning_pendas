...
def dedupek12():
    ...
    school_df.to_csv(f'{state} - school_df - INITIAL{today}.csv', index=False, float_format='%.0f')
    district_df.to_csv(f'{state} - district_df - INITIAL{today}.csv', index=False, float_format='%.0f')
...
#         duplicate_contacts_from_mark_to_delete_email_opt_out.to_csv(f'duplicate_contacts{today}.csv', index=False, float_format='%.0f')
...
#     merged_school_accounts_with_salesforce_data = deduped_main_school_df.merge(school_accounts_by_state, on =['Nces School Id'],how='outer', indicator=True)
#     merged_school_accounts_with_salesforce_data.to_csv(f'{state} - School Account Merge{today}.csv', index=False, float_format='%.0f')
...
#     merged_district_accounts_with_salesforce_data.to_csv(f'{state} - District Account Merge{today}.csv', index=False, float_format='%.0f')
...
#     school_accounts_not_in_salesforce.to_csv(f'{state} - Schools Accounts To Import{today}.csv', index=False, float_format='%.0f')
...
def createpublicschools():
    ...
    # schooldata = schooldata[~schooldata['District Id'].isna()]
    ...
    for file in os.listdir(response):
        if 'success' in file and file.endswith('.csv'):
            ...
            dc.rename(columns={'DISTRICT ID':'District Id'},inplace=True)
            # dc['District Id']=dc['District Id'].astype(int)

    school_accounts_with_salesforce_account = dc.merge(schooldata, on =['District Id'],how='right')
    ...
    school_accounts_with_salesforce_account.to_csv(f'Public School to Import{today}.csv', index=False, float_format='%.0f')
...
def matchcontacts():
    ...
    districtcontactdata = session.get('districtcontactdata')
    districtcontactdata = pd.read_json(districtcontactdata, dtype=False)
    districtcontactdata.to_csv(f'district_contacts2_df{today}.csv', index=False, float_format='%.0f')
    ...
    schoolcontactdata.to_csv(f'school_contacts_df{today}.csv', index=False, float_format='%.0f')
    ...
    districtcontactdata.to_csv(f'district_contacts3_df{today}.csv', index=False, float_format='%.0f')
    schoolcontactdata.to_csv(f'school_contacts2_df{today}.csv', index=False, float_format='%.0f')

    all_contacts = district.dedupeContactData(all_contacts)

    all_contacts.to_csv(f'allcontact_df{today}.csv', index=False, float_format='%.0f')
